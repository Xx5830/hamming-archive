...
managefile::Hamarc::EncodingInfo::EncodingInfo()
    : count_data_bits(0), count_data_bytes(0), count_hemming_bits(0), count_additional_bytes(0), count_copy(0) {}
...
managefile::Hamarc::ConfigEncodingFile::ConfigEncodingFile(EncodingInfo sf, EncodingInfo snf, EncodingInfo nf,
                                                           EncodingInfo edf)
...
managefile::Hamarc::Hat::Hat(std::span<char> sf, std::span<char> snf, std::span<char> nf, std::span<char> edf) {
...
managefile::Hamarc::Hat::Hat() : size_file(0), size_name_file(0), encoding_data_file(0) {}
...
std::vector<uint8_t> managefile::Hamarc::HemmingBytes(std::span<char> data, const EncodingInfo& info) const {
    std::vector<uint8_t> hbytes(info.count_additional_bytes + (info.count_data_bits % 8 != 0 ? 1 : 0), 0);
...
std::vector<char> managefile::Hamarc::EncodeBlock(std::span<char> block, const EncodingInfo& info) const {
    std::vector<char> result(block.begin(), block.end());

    std::vector<uint8_t> hbytes = HemmingBytes(result, info);
    size_t ih = 0;

    for (size_t bit = info.count_data_bits % 8; bit != 0 && bit < 8 && ih < info.count_hemming_bits; ++bit, ++ih) {
        uint8_t hbit = (hbytes[ih / 8] >> (ih % 8)) & 1;
        result.back() = static_cast<char>(static_cast<uint8_t>(result.back()) | (hbit << bit));
    }
...
std::vector<char> managefile::Hamarc::EncodeData(std::span<char> data, const EncodingInfo& info) const {
    struct Splitter {
        ...
        void Feed(std::span<char> d) {
            for (char byte : d) {
                for (size_t b = 0; b < 8; ++b, ++cur_bit) {
                    ...
                    if (byte & (1 << b)) {
                        current.back() = static_cast<char>(static_cast<uint8_t>(current.back()) | (1 << (cur_bit % 8)));
                    }
                }
            }
        }
        ...
        std::vector<std::vector<char>> Finish(size_t count_data_bytes) {
            ...
            if (blocks.empty()) {
                blocks.push_back(std::vector<char>(count_data_bytes, 0));
            }
...
managefile::File managefile::Hamarc::EncodeFile(File& src_file, const EncodingInfo& enc_info, size_t buff_size) const {
    ...
    auto lcm = [&gcd](size_t a, size_t b) -> size_t { return a / gcd(a, b) * b; };
...
std::vector<char> managefile::Hamarc::DecodeBlock(std::span<char> block, const EncodingInfo& info) const {
    ...
    if (info.count_data_bits % 8 != 0) {
        ...
        for (size_t bit = 0; bit < info.count_data_bits % 8; ++bit) {
            data.back() = static_cast<char>(static_cast<uint8_t>(data.back()) |
                                            (static_cast<uint8_t>(block[data.size() - 1]) & (1 << bit)));
        }

        hbytes_stored.push_back(0);
        for (size_t bit = info.count_data_bits % 8; bit < 8 && ih < info.count_hemming_bits; ++bit, ++ih) {
    ...
    if (cnt_diff > 1 && syndrome > 0) {
        ...
        if (byte_idx < data.size()) {
            data[byte_idx] =
                static_cast<char>(static_cast<uint8_t>(data[byte_idx]) ^ static_cast<uint8_t>(1 << bit_idx));
        }
    }
...
std::vector<char> managefile::Hamarc::DecodeData(std::span<char> data, const EncodingInfo& info) {
...
managefile::Hamarc::Hat managefile::Hamarc::DecodeHat(size_t pos) {
    ...
    auto read_field = [&](size_t count_bytes, const EncodingInfo& cfg) {
        size_t encoded_bytes = count_bytes * (cfg.count_data_bytes + cfg.count_additional_bytes) * cfg.count_copy;
    ...
    size_t enc_name_bytes = name_len * (config_.name_file.count_data_bytes + config_.name_file.count_additional_bytes) *
                            config_.name_file.count_copy;
...
managefile::File managefile::Hamarc::DecodeFile(size_t pos, const Hat& hat, size_t buff_size) {
    ...
    EncodingInfo enc(hat.encoding_data_file >> 8, static_cast<uint32_t>((hat.encoding_data_file) & 0xFF));

    size_t size_block = (enc.count_data_bytes + enc.count_additional_bytes) * enc.count_copy;
    if (size_block == 0)
        return tmp;

    buff_size = std::max(buff_size, size_block);
...
size_t managefile::Hamarc::GetPosFirstFile() const { return begin_file_pos_; }
...
managefile::Hamarc::Hamarc(const std::string& file_path, bool create_if_not_exist, const ConfigEncodingFile& config)
    ...
    if (std::filesystem::exists(file_path) && std::filesystem::file_size(file_path) > 0) {
        ...
        auto push = [&](const std::vector<char>& v) { config_bytes.insert(config_bytes.end(), v.begin(), v.end()); };
...
managefile::Hamarc::ConfigEncodingFile managefile::Hamarc::GetConfig() const { return config_; }
...
size_t managefile::Hamarc::Length() const { return count_file_; }
...
std::vector<managefile::File> managefile::Hamarc::GetAll() {
    ...
    for (size_t i = 0; i < count_file_; ++i) {
        ...
        size_t next_pos = data_pos + GetSizeFile(hat);
